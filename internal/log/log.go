...
func GetLogger() *zap.SugaredLogger {
	...
	infoFileWriteSyncer := zapcore.AddSync(&lumberjack.Logger{
		Filename:   "../storage/log/info.log", //日志文件存放目录，如果文件夹不存在会自动创建
	...
	errorFileWriteSyncer := zapcore.AddSync(&lumberjack.Logger{
		Filename:   "../storage/log/error.log", //日志文件存放目录
	...
	return zap.New(zapcore.NewTee(infoFileCore, errorFileCore), zap.AddCaller()).Sugar() //zap.AddCaller()为显示文件名和行号
}
